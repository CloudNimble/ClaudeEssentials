...
```csharp
public CloudNimble.ClaudeEssentials.Hooks.PermissionRequestBehavior Behavior { get; set; }
```
...
Gets or sets a value indicating whether to interrupt the current operation.
            Only applicable when `Behavior` is [PermissionRequestBehavior.Deny](/api-reference/CloudNimble/ClaudeEssentials/Hooks/PermissionRequestBehavior#deny).
...
Gets or sets optional modifications to the tool's input parameters.
            Only applicable when `Behavior` is [PermissionRequestBehavior.Allow](/api-reference/CloudNimble/ClaudeEssentials/Hooks/PermissionRequestBehavior#allow).
